...
def load_frame_json(p: Path) -> Optional[np.ndarray]:
    ...
    def pick3d2d(k3: str, k2: str):
        ...
        # 'people' 안에 없으면, json 루트 레벨에서 찾기 (구형 포맷 대비)
        if v3 is None and v2 is None:
            v3 = d.get(k3, None)
            v2 = d.get(k2, None)
            
        # 3D 데이터가 존재하고 비어있지 않으면 3D 우선
    ...
    def take_coords(vec: np.ndarray, C: int) -> Optional[np.ndarray]:
        """ [x,y,c, x,y,c] 또는 [x,y,z,c, ...] 리스트를 [N, C] 배열로 변환 """
        if C not in (2, 3):
            return None
...
def collect_frames(first_json: Path) -> Tuple[Optional[str], List[Path]]:
    ...
    # v4 스크립트는 ...F_000000..._keypoints.json 파일을 찾음
    # 여기서 stem을 ...F로 추출해야 함
    
    # 1. 정규식 시도 (NIA_SL_WORD..._keypoints.json)
    m = STEM_RE.match(first_json.name)
    if m:
        stem = m.group(1)  # 공통 stem (예: 'NIA_SL_WORD0001_REAL01F')
    else:
        # 2. 정규식 실패 시 (예: 프레임 번호가 없는 경우), stem 자체를 사용
        stem = normalize_stem(first_json.stem)
        
    pattern = f"{stem}_" + "[0-9]"*12 + "_keypoints.json"
    frames = sorted(first_json.parent.glob(pattern))
    
    # 3. glob 패턴 실패 시 (프레임 번호가 없는 단일 파일)
    if not frames and first_json.exists():
        return stem, [first_json]
...
def main():
    ...
    for i, row in iterator:
        ...
        # build_index.py (v4/v5)는 D:\ub\ubiplay\data\keypoints\... 경로를 찾음
        # 이 경로는 data_root (D:\ub\ubiplay) 또는 relative-to (D:\ub\ubiplay) 기준으로
        # 올바르게 해석되어야 함.
        ...
        if not frames:
            npy_paths.append(None); fail_reason.append(f"no_frames_found_for: {kp_path.name}"); continue
        ...
        try:
            # .npy 파일은 out_root 바로 아래에 저장되므로, stem.npy만 경로가 됨
            npy_relative_path = Path(f"{stem}.npy")
        except ValueError:
            npy_relative_path = npy_file_path # 상대 경로화 실패 시 절대 경로
            
        # PC 이동을 고려, 경로 구분자를 '/'로 통일
        npy_path_str = norm_sep(npy_relative_path)
        ...
        try:
            ...
            npy_paths.append(None); fail_reason.append(f"save_failed: {e}")
